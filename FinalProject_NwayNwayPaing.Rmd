...
```{r}
# Table
table1(~ age + sex + race + education + marital_new + smoking + alcohol + phy_act + bmi_cat | predia_binary,overall="Total", footnote='SD = standard deviation', data = pre_diabetes_ex)
...
### We excluded a lot of people with pre-diabetes. Then, we will exclude observations based on the Cook's D graph and (>.0035).
```{r}
...
```
The results from model4 and model 2 are the same.
